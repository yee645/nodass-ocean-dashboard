...
- 這是「棲地適合度/出現潛勢」，**不是漁獲量**；目前**無 CPUE 標籤**，無法預測魚量。查證過台灣公開資料
  （漁船進出港紀錄、VMS/作業日誌、月別港口產量）都拿不到可用的漁獲量/努力量標籤，非短期能補的缺口。
- 衛星 archive 止於約 2021；「今日/未來」高解析靠 CWA OCM 數值預報；葉綠素預報用氣候平均（非逐日真實預報，
  故「未來」模式逐天差異主要來自水溫/海流變化，葉綠素項每天數值接近）。
- **「未來」模式的時間預測未經驗證**：空間分塊 AUC 驗證的是「同一時間點下模型能否排出高低機率區域」，
  不是「+N 天後的預測是否成真」。要驗證後者需要「未來實際出現」的真實資料對照時間點，目前沒有這種資料，
  故「明日預報這裡熱、後天真的會熱」是模型假設（環境條件變化 → 適合度隨之變化的合理外推），未經事實驗證，
  對外溝通勿宣稱「預測未來 X 天魚群位置」已驗證準確，應講「依預報環境條件推算的潛勢區域」。
- 「魚群熱區與漂移」的漂移箭頭＝該熱區涵蓋格子的**洋流向量平均**（`hotspotModel.ts` 的 `buildCore()`），
  是「假設隨水流被動漂移」的視覺提示，**不是**用真實魚群移動/標識放流資料訓練出的移動預測，魚會主動洄游、
  逆流覓食，不等於洋流方向，對外溝通不可稱為「魚群移動預測」。
- **已實測「換成真正訓練的分類器」不會更準，是樣本量問題，非模型選型問題**：`train_sdm_model.py` 拿
  `build_sdm_dataset.py` 統整出的訓練集，對 17 種魚各自訓練 QDA、多項式邏輯迴歸（皆為能表達彎曲/有界
  決策邊界的模型，跟現行包絡模型同一形狀假設），同樣走 leave-one-block-out 空間 CV，結果平均 AUC 只有
  0.61，輸給現行高斯包絡模型的 0.80（見 `sdm/sdm_model_comparison.csv`）。原因非模型形狀選錯（in-sample
  不做 CV 時新模型其實跟得上甚至略好），而是判別式分類器需要學邊界、樣本量吃緊：leave-one-block-out
  每折要扣掉一整個空間分塊，某些魚種一折就損失三分之一出現點，20–71 筆的量級撐不住；包絡模型只需估
  平均值+共變異數（3 特徵僅 9 參數），天生對小樣本穩健。**結論：目前資料量下，現行包絡模型就是合適的
  複雜度，不建議升級判別式/樹模型，除非出現點資料量大幅增加**（見待辦 P4，卡在拿不到 CPUE 級標籤）。
